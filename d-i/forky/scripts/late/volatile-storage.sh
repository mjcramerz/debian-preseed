...
install_target_tmpfs_pre_clean_assets() {
  ...
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_SHARED_TARGET etc/systemd/system/tmp.mount.d/override.conf)" \
    "${FILE_TMPFS_PRE_CLEAN_TMP_MOUNT_OVERRIDE}" \
    0644
  stage_target_tmpfs_pre_clean_mount_override_if_enabled TMPFS_DEV_SHM \
    "$(installer_repo_join_var DIR_HOOKS_SHARED_TARGET etc/systemd/system/dev-shm.mount.d/override.conf)" \
