...
write_target_kernel_tunables() {
  ...
  stage_target_asset_if_path "${DISK_HOOK_ROOT}/target/etc/default/grub.d/72-nvme-platform.cfg" "${FILE_GRUB_NVME_DROPIN:-}" "${DIR_GRUB_DEFAULT}/72-nvme-platform.cfg" 0644
  stage_target_asset_if_path "${DISK_HOOK_ROOT}/target/etc/default/grub.d/73-pcie-power.cfg" "${FILE_GRUB_PCIE_POWER_DROPIN:-}" "${DIR_GRUB_DEFAULT}/73-pcie-power.cfg" 0644
  stage_target_asset_if_path "${CPU_HOOK_ROOT}/target/etc/default/grub.d/74-intel-mei-blacklist.cfg" "${FILE_GRUB_MEI_BLACKLIST_DROPIN:-}" "${DIR_GRUB_DEFAULT}/74-intel-mei-blacklist.cfg" 0644
  stage_target_asset_if_path "${CPU_HOOK_ROOT}/target/etc/default/grub.d/75-intel-vfio.cfg" "${FILE_GRUB_VFIO_DROPIN:-}" "${DIR_GRUB_DEFAULT}/75-intel-vfio.cfg" 0644
