...
installer_loaded_repo_dir_value() {
  ...
  case "${1:-}" in
    ...
    DIR_HOOKS_SHARED_POST_BASE_INSTALLER_D) printf '%s\n' "${DIR_HOOKS_SHARED_POST_BASE_INSTALLER_D:-}" ;;
    ...
    DIR_HOOKS_SHARED_PRE_PKGSEL_D) printf '%s\n' "${DIR_HOOKS_SHARED_PRE_PKGSEL_D:-}" ;;
...
hooks/shared/post-base-installer.d|DIR_HOOKS_SHARED_POST_BASE_INSTALLER_D|
hooks/shared/partman/finish.d|DIR_HOOKS_SHARED_PARTMAN_FINISH_D|
hooks/shared/partman|DIR_HOOKS_SHARED_PARTMAN|
hooks/shared/pre-pkgsel.d|DIR_HOOKS_SHARED_PRE_PKGSEL_D|
hooks/shared/target|DIR_HOOKS_SHARED_TARGET|
hooks/shared|DIR_HOOKS_SHARED|
...
installer_target_apt_preferences_source_variant() {
  if installer_selected_class_reference_is_selected addon/server-suite 2>/dev/null &&
    installer_selected_class_reference_is_selected role/server 2>/dev/null
  then
    printf '%s\n' server
  else
    printf '%s\n' desktop
...
installer_target_apt_preference_source_path() {
  ...
  printf '%s\n' "$(installer_repo_join_var DIR_HOOKS_SHARED_TARGET "etc/apt/preferences.d/${pref_variant}/${pref_name}")"
}
...
installer_fetch_url() {
  ...
  fetch_url_err_path="${fetch_url_tmp_path}.wget.err"
...
installer_copy_seed_file() {
  ...
  copy_seed_err_path="${copy_seed_tmp_path}.copy.err"
...
installer_fetch_host_env() {
  ...
  set -- \
    "$(installer_repo_join_var DIR_HOSTS_PROFILES "${host_profile_env_dir}/${host_profile_env_name}.env")" \
  ...
  case "$host_variant" in
    desktop)
      ;;
    server)
      set -- "$@" "$(installer_repo_join_var DIR_HOSTS_SHARED server.env)"
      ;;
    *)
