...
bootstrap_fetch_seed_file() {
  bootstrap_fetch_seed_base=$1
  bootstrap_fetch_src=$2
  bootstrap_fetch_dest=$3
  bootstrap_fetch_mode=${4:-0600}
  bootstrap_fetch_label=${5:-file}
  bootstrap_fetch_timeout_seconds=${6:-45}

  case "$bootstrap_fetch_src" in
    "$(bootstrap_repo_env_relpath)") ;;
    *)
      bootstrap_ensure_repo_env "$bootstrap_fetch_seed_base"
      bootstrap_fetch_seed_base=$1
      bootstrap_fetch_src=$2
      bootstrap_fetch_dest=$3
      bootstrap_fetch_mode=${4:-0600}
      bootstrap_fetch_label=${5:-file}
      bootstrap_fetch_timeout_seconds=${6:-45}
      bootstrap_fetch_src=$(bootstrap_repo_resolve_path "$bootstrap_fetch_src")
      ;;
  esac

  bootstrap_fetch_seed_type=$(bootstrap_seed_source_type "$bootstrap_fetch_seed_base")
  bootstrap_fetch_cache_path=$(bootstrap_seed_cache_path "$bootstrap_fetch_seed_base" "$bootstrap_fetch_src")

  if [ ! -s "$bootstrap_fetch_cache_path" ]; then
    bootstrap_log "info: seed cache miss for ${bootstrap_fetch_src}; fetching ${bootstrap_fetch_label} from ${bootstrap_fetch_seed_type} seed source"
    bootstrap_fetch_tmp_dest="${bootstrap_fetch_cache_path}.tmp.$$"
    bootstrap_fetch_log="${bootstrap_fetch_tmp_dest}.fetch.err"
    bootstrap_fetch_parent_dir=$(dirname "$bootstrap_fetch_cache_path")

    [ -d "$bootstrap_fetch_parent_dir" ] || install -d -m 0700 "$bootstrap_fetch_parent_dir"
    rm -f "$bootstrap_fetch_tmp_dest" "$bootstrap_fetch_log"

    case "$bootstrap_fetch_seed_type" in
      file)
        bootstrap_fetch_seed_source_path="${bootstrap_fetch_seed_base%/}/$bootstrap_fetch_src"
        if cp "$bootstrap_fetch_seed_source_path" "$bootstrap_fetch_tmp_dest" >"$bootstrap_fetch_log" 2>&1; then
          bootstrap_fetch_status=0
        else
          bootstrap_fetch_status=$?
        fi
        ;;
      url)
        if wget --no-verbose --tries=3 --timeout="$bootstrap_fetch_timeout_seconds" -O "$bootstrap_fetch_tmp_dest" "$bootstrap_fetch_seed_base/$bootstrap_fetch_src" >"$bootstrap_fetch_log" 2>&1; then
          bootstrap_fetch_status=0
        else
          bootstrap_fetch_status=$?
        fi
        ;;
      *)
        bootstrap_fatal "unsupported installation source type: $bootstrap_fetch_seed_type"
        ;;
    esac

    if [ "$bootstrap_fetch_status" -ne 0 ]; then
      bootstrap_log "error: failed to fetch ${bootstrap_fetch_label}: ${bootstrap_fetch_src} (status ${bootstrap_fetch_status})"
      [ -s "$bootstrap_fetch_log" ] && sed "s/^/[bootstrap:${bootstrap_fetch_seed_type}] /" "$bootstrap_fetch_log" >&2
      rm -f "$bootstrap_fetch_tmp_dest" "$bootstrap_fetch_log"
      exit 1
    fi
    rm -f "$bootstrap_fetch_log"
    if [ ! -s "$bootstrap_fetch_tmp_dest" ]; then
      rm -f "$bootstrap_fetch_tmp_dest"
      bootstrap_fatal "fetched ${bootstrap_fetch_label} is empty: ${bootstrap_fetch_src}"
    fi
    mv "$bootstrap_fetch_tmp_dest" "$bootstrap_fetch_cache_path"
    chmod 0600 "$bootstrap_fetch_cache_path"
  else
    bootstrap_log "info: seed cache hit for ${bootstrap_fetch_src}"
  fi

  bootstrap_copy_path_with_mode \
    "$bootstrap_fetch_cache_path" \
    "$bootstrap_fetch_dest" \
    "$bootstrap_fetch_mode" \
    "cached seed file ${bootstrap_fetch_src}"
}
